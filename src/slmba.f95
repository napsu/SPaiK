...
MODULE slmba_mod  ! Stochastic Limited memory bundle algorithm
    ...
    ! MODULE slmba_mod includes the following subroutines (S) and functions (F).
    PUBLIC :: &
        slmba, &      ! S Limited memory bundle subroutine for nonsmooth
...
CONTAINS
    ...
    SUBROUTINE slmba(x,n,mc,f,nit,nfe,nge,iterm,seed)

        USE param, ONLY : small,large,zero,half,one
        USE initpkl, ONLY : &
            ri=>r, &         ! Indices, r(n);
            si=>s, &         ! Indices, s(n);
            rbatch, &        ! Indices, rbatch(n);
            sbatch, &        ! Indices, sbatch(n);
            bi, &            ! Batch indices, bi(n);
            nb, &            ! Size of the batch, nb <= n;
            m                ! Number of unique drugs (or targets, in target-wise batching).
        USE initslmba, ONLY : &
        ...
        INTEGER :: i, &
            nitb, &      ! Number of iterations within batch.
        ...
        integer :: j,k,itmp,ibatch,fcount,inth
        ...
        ! Compute batch indices from targets (or drugs)
        if (m /= nbatch) then
            do i = 1,nbatch 
                call random_number(urand)
                j = i + FLOOR((m+1-i)*urand)
                itmp = batchind(i)
        ...
        batch: DO ibatch = 1,maxbatch
            ...
            iteration: DO
                ...
                IF (iters > 0) THEN ! tähän myös pred?
            ...
            if (m /= nbatch) then
                if (batchtype == 1) then
                    ! batches such that all indices are used at least once (not exactly if not m is not divident with nbatch)
                    ib = ib+nbatch
                    if (ib > m-nbatch) then
                        ib = 0  !
                    end if    
        ...
        if (iterm == -7 .OR. iterm == -3) then 
            PRINT*,' Something is wrong in function value, f = ',f ! Note, this never happened in tests.
            RETURN ! Fatal failure, stop computing.
        end if
